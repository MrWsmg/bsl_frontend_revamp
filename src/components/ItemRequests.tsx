...
import { CheckCircle, XCircle, Clock, Filter, Package, User, Calendar, Wheat } from 'lucide-react';
import { toast } from 'sonner';
import apiService from '../services/api';
import { ItemRequest, Farm } from '../types';
...
export default function ItemRequests({ farms }: ItemRequestsProps) {
  ...
  const handleApproveRequest = async (requestId: number) => {
    const confirmApprove = window.confirm('Are you sure you want to approve this item request? This will add the item to inventory.');
    if (!confirmApprove) return;

    const loadingToast = toast.loading('Approving request...');
    try {
      await apiService.approveItemRequest(requestId);
      toast.success('Request approved and item added to inventory!', { id: loadingToast });
      loadRequests();
    } catch (error: any) {
  ...
  const handleRejectRequest = async (requestId: number) => {
    const confirmReject = window.confirm('Are you sure you want to reject this item request?');
  ...
  const handleIssueRequest = async (requestId: number) => {
    const confirmIssue = window.confirm('Are you sure you want to issue this item? This will deduct from inventory.');
    if (!confirmIssue) return;

    const loadingToast = toast.loading('Issuing item...');
    try {
      await apiService.issueItemRequest(requestId);
      toast.success('Item issued successfully!', { id: loadingToast });
      loadRequests();
    } catch (error: any) {
      console.error('Error issuing request:', error);
      toast.error(error.response?.data?.detail || 'Failed to issue item. Please try again.', { id: loadingToast });
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'not_received':
        return <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100">Not Received</Badge>;
      default:
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Item Requests Management</h2>
          <p className="text-muted-foreground">
            Review and approve item requests from supervisors
          </p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center">
            ...
            <p className="text-muted-foreground">
              {statusFilter === 'all' ? 'All item requests have been processed.' : `No ${statusFilter} requests found.`}
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={request.id} className="overflow-hidden">
                <CardContent className="p-0">
                  <div className="flex flex-col lg:flex-row">
                    ...
                    <div className={`lg:w-1.5 h-1.5 lg:h-auto ${
                      request.status === 'approved' ? 'bg-green-500' :
                      request.status === 'rejected' ? 'bg-red-500' :
                      request.status === 'issued' ? 'bg-blue-500' :
                      request.status === 'received' ? 'bg-green-600' :
                      request.status === 'not_received' ? 'bg-orange-500' :
                      'bg-yellow-500'
                    }`} />
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-3 bg-muted/50 rounded-lg">
                            <div>
                              ...
                              <p className="text-lg font-bold mt-1">{request.quantity} {request.unit}</p>
                            </div>
                            <div>
                          ...
                          {request.approved_at && (
                            <div className="mt-3 text-sm text-muted-foreground bg-muted/50 p-2 rounded-md">
                              ...
                              {' '}by {request.approver_name || 'Unknown'} on{' '}
